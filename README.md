...
### 激活步骤（管理员命令执行）
1. 卸载原来的密钥`slmgr /upk`
2. 设置服务器 `slmgr -skms kms.org.cn`
3. 安装密钥 `slmgr -ipk 版本对应密钥`
4. 激活系统 `slmgr -ato`

### 可用服务
+ `ping kms.org.cn ` 测试是否可用
+ `telnet kms.org.cn 1688` 测试服务是否可用


### 查询对应密钥 
[点击查询微软公开通用批量许可证密钥](https://learn.microsoft.com/zh-cn/windows-server/get-started/kms-client-activation-keys)

### Windows VL 密钥对照表(以下内容来自微软官方公开的通用批量许可证密钥，请放心使用)
...
2. 注册 KMS 服务 `cscript ospp.vbs /sethst:kms.org.cn`
